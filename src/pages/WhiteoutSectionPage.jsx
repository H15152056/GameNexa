...
function GenericCollectionPage({ title, icon, description, items = [], emptyText = 'No content available yet.' }) {
  const safeItems = Array.isArray(items) ? items : []

  return (
    <>
      <div className="wo-result-count">
        Showing <b>{safeItems.length}</b> items
      </div>

      {safeItems.length ? (
        <div className="wo-tools-grid">
          {safeItems.map((item, index) => {
            const name = item?.name || item?.title || item?.label || `Item ${index + 1}`
            const type = item?.type || item?.role || item?.category || ''
            const desc = item?.description || item?.desc || item?.content || 'GameNexa Whiteout Survival information.'
            const image = item?.image || item?.img || item?.icon || ''

            return (
              <article className="wo-quick-card wo-content-card" key={item?.id || item?.slug || `${name}-${index}`}>
                <div className="wo-quick-icon">
                  {image ? <img src={image} alt="" /> : icon}
                </div>
                <div className="wo-quick-content">
                  <strong>{name}</strong>
                  {type ? <span className="wo-content-type">{type}</span> : null}
                  <span>{desc}</span>
                </div>
              </article>
            )
...
export default function WhiteoutSectionPage() {
  ...
  const cmsGames = useCMSGamesData(gamesData)
  const game = cmsGames.find((item) => item.slug === 'whiteout-survival') || gamesData.whiteoutSurvival

  const isMaps = currentSection === 'battle-maps' || currentSection === 'maps' || currentSection === 'map'
  const isHeroes = currentSection === 'heroes' || currentSection === 'heros'
  const isCalculators = currentSection === 'calculators' || currentSection === 'calculator'
  const isPlanner = currentSection === 'planner' || currentSection === 'alliance-planner'
  ...
  const collectionPages = {
    facilities: ['Facilities', ICONS.facility, game?.facilities],
    fortresses: ['Fortresses', ICONS.fortress, game?.fortresses],
    strongholds: ['Strongholds', ICONS.stronghold, game?.strongholds],
    resources: ['Resources', ICONS.resource, game?.resources],
    events: ['Events', ICONS.event, game?.events],
    buildings: ['Buildings', '🏗️', game?.buildings],
    research: ['Research', '🔬', game?.research],
    troops: ['Troops', ICONS.troop, game?.troops],
    beartrap: ['Bear Trap', ICONS.battle, game?.beartrap],
    'bear-trap': ['Bear Trap', ICONS.battle, game?.beartrap],
  }

  if (collectionPages[currentSection]) {
    const [title, icon, items] = collectionPages[currentSection]
    return (
      <main className="wo-page-shell">
        <style>{PAGE_STYLES}</style>
        <SectionHero
          ...
          description={`Explore GameNexa's Whiteout Survival ${title.toLowerCase()} database and strategy information.`}
        />
        <div className="wo-breadcrumb">
          <Link to="/game/whiteout-survival/">Whiteout Survival</Link>
          <span>{ICONS.arrow}</span>
        ...
        <GenericCollectionPage title={title} icon={icon} description={`No ${title.toLowerCase()} have been added yet.`} items={items} />
        <GuidesLinkSection />
      </main>
    )
  }

  if (currentSection === 'guides' || currentSection === 'guide') {
    return (
      <main className="wo-page-shell">
        <style>{PAGE_STYLES}</style>
        <SectionHero icon={ICONS.guide} title="Whiteout Survival Guides" description="Practical guides for heroes, events, progression and alliance strategy." />
        <div className="wo-breadcrumb"><Link to="/game/whiteout-survival/">Whiteout Survival</Link><span>{ICONS.arrow}</span><strong>Guides</strong></div>
        <GenericCollectionPage title="Guides" icon={ICONS.guide} description="Guides are managed through the GameNexa CMS." items={game?.guides || []} />
      </main>
    )
  }
...
.wo-content-card{min-height:104px;align-items:flex-start}.wo-content-card .wo-quick-icon img{width:42px;height:42px;object-fit:cover;border-radius:10px}.wo-content-type{font-weight:800!important;color:#9ecde1!important;font-size:10px!important}
